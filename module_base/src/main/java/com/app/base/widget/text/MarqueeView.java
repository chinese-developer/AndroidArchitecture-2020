...
public class MarqueeView extends View implements Runnable {
    ...
    private void initattrs(AttributeSet attrs) {
        TintTypedArray tta = TintTypedArray.obtainStyledAttributes(getContext(), attrs,
                R.styleable.ArchMarqueeView);

        textColor = tta.getColor(R.styleable.ArchMarqueeView_arch_marquee_text_color, textColor);
        isClickStop = tta.getBoolean(R.styleable.ArchMarqueeView_arch_marquee_is_clickable_stop, isClickStop);
        isResetLocation = tta.getBoolean(R.styleable.ArchMarqueeView_arch_marquee_is_reset_location, isResetLocation);
        speed = tta.getFloat(R.styleable.ArchMarqueeView_arch_marquee_text_speed, speed);
        textSize = tta.getFloat(R.styleable.ArchMarqueeView_arch_marquee_text_size, textSize);
        textDistance1 = tta.getInteger(R.styleable.ArchMarqueeView_arch_marquee_text_distance, textDistance1);
        startLocationDistance = tta.getFloat(R.styleable.ArchMarqueeView_arch_marquee_text_startlocationdistance, startLocationDistance);
        repeatType = tta.getInt(R.styleable.ArchMarqueeView_arch_marquee_repeat_type, repeatType);
        tta.recycle();
    }
...
//                    此时停止线程就可以了
    ...
    public void setTextDistance(int textdistance2) {
        ...
        setText(content);//设置间距以后要重新刷新内容距离，不过如果内容是List形式的，该方法不适用
    }
