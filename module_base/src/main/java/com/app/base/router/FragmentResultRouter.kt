...
/**
 * https://github.com/xybean/FragmentARouter
 *
 ...
 */
class FragmentResultRouter private constructor(private val fragment: androidx.fragment.app.Fragment) {

    companion object {
        private val router = ARouter.getInstance()
        private var mHandler: Handler = Handler(Looper.getMainLooper())

        fun build(fragment: androidx.fragment.app.Fragment, path: String): FragmentResultRouter {
            val fragmentRouter = FragmentResultRouter(fragment)
            fragmentRouter.postcard = router.build(path)
            return fragmentRouter
        }

        fun build(fragment: androidx.fragment.app.Fragment, uri: Uri): FragmentResultRouter {
            val fragmentRouter = FragmentResultRouter(fragment)
                ...
                mHandler.post {
                    ...
                    if (-1 != postcard.enterAnim && -1 != postcard.exitAnim) {    // Old version.
                        fragment.requireActivity()
                            .overridePendingTransition(postcard.enterAnim, postcard.exitAnim)
                    }
                }
            } else {
                fragment.startActivityForResult(intent, requestCode, postcard.optionsBundle)
    ...
    fun withFlags(flag: Int): FragmentResultRouter {
