...
public class NetStateReceiver extends BroadcastReceiver {
    ...
    private static NetworkState sStatus = null;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (null != state_wifi && State.CONNECTED == state_wifi) { // 判断是否正在使用WIFI网络
            ...
            Timber.d("sStatus=" + sStatus + "改变后的网络为WIFI");
        } else if (null != state_gprs && State.CONNECTED == state_gprs) { // 判断是否正在使用GPRS网络
            tempStatus = STATE_GPRS;
            Timber.d("sStatus=" + sStatus + "改变后的网络为GPRS");
        } else {
            tempStatus = STATE_NONE;
            Timber.d("sStatus=" + sStatus + "改变后的网络为无连接");
        }

        if (sStatus != tempStatus) {
